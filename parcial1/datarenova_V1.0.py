datarenova v1.0
# El programa analizará cierta cantidad de datacenters para luego revisar si cumple con una cantidad mínima de elementos,
# si esta cantidad mínima de elementos no se cumple el datacenter que incumpla lo establecido será renovado. Además se mostrará
# un reporte con la suma total de elementos y el promedio por elemento de todos los datacenters.

# Desarrollado por Juan José Escobar
...
nombre_archivo_registro = "registro.txt"
nombre_archivo_reporte = "reporte.txt"
nombre_archivo_renovar = "datas_a_renovar.txt"
...
# 1 -> Funcion que crear un archivo con el nombre especificado. Devuelve True si fue exitoso o False en caso de error.
...
# 2-> Funcion que lee las lineas de un archivo de texto y las devuelve en una lista.
...
# 3-> Funcion que guarda al final del archivo definido la linea especificada. Devuelve True si fue exitoso o False en caso de error.
...
# 4-> Funcion que guarda un mensaje de error en el archivo errores.txt
def guardar_error(mensaje_error):
	escribir_linea_archivo(nombre_archivo_errores, mensaje_error + "\n")
	
# 5-> Funcion que guarda un registro de operacion en el archivo log.txt
...
# 7-> Validacion de estructura de la cantidad mínima de elementos.
...
def validar_descripcion(nombre_archivo):
	...
	if (arreglo_prop1[0].lower() == "ciudad" and arreglo_prop2[0].lower() == "ingeniero a cargo") == False :
		guardar_error("Las propiedades del archivo " + str(nombre_archivo) + " no son validas, debe ser ciudad e ingeniero a cargo")
		return False
...
# 8->Funcion que finaliza el programa y guarda el respectivo mensaje de terminacion en el archivo errores.txt
...
guardar_log("Creados archivos registro.txt y errores.txt")
...
# Se valida que exista el archivo elementos.txt, en la misma direccion donde esta datarenova (Va2)
...
# Se valida que exista el archivo restricciones.txt, en la misma direccion donde esta datarenova (Va2) y (Va7)
if os.path.isfile(nombre_archivo_minimos) == False:
	terminar_programa("El archivo 'restricciones' no existe, y si existe, no tiene la extension .txt valida")

guardar_log("Archivo 'restricciones' si existe y su extension es correcta")
...
guardar_log("Numero de criterios en elementos bien :P")
guardar_log("Archivo de elementos leido")

# Se leen las lineas del archivo reestricciones y se almacenan en la variable lineas_archivo_minimos
...
# Se valida que en restricciones hayan el mismo numero de elementos que en dicho archivo. (Va5)
if numero_lineas_minimos != numero_lineas_elementos :
	terminar_programa("El numero de criterios de elementos no coincide con el numero de criterios de restricciones")

 
guardar_log("El numero de criterios en elementos y restricciones OK")
guardar_log("Archivo de restricciones leido")
...
# Encabezado para elementos y para restricciones
tabla_total[0][0] = "Elementos"
tabla_total[0][1] = "Minimos"

guardar_log("Cargando elementos y restricciones en memoria")

# Inicio ciclo 1 -> x, en este ciclo se agregan al arreglo tabla_total las 2 primeras columnas que corresponden a los elementos
# del archivo elementos y a los minimos del archivo restricciones, asi como al mismo tiempo se va validando las lineas de cada 
# archivo.
for x in range(0,numero_lineas_elementos):
	...
	guardar_log("	Procesando linea " + str(x+1) + " de minimos")
	
	# Se valida la estructura de las lineas del archivo restricciones con la funcion validar_lineas (Va8)
	linea_validada_minimos = validar_lineas(lineas_archivo_minimos[x], x+1, nombre_archivo_minimos)
	if linea_validada_minimos[0] == False:
		terminar_programa("Error en archivo 'restricciones.txt'")
	
	# Se valida que el elemento de la misma linea de restricciones.txt coincida con el elemento de la misma linea de elementos.txt
	...
	if (linea_validada_exhibicion[1] == linea_validada_elementos[1]):
		...
		terminar_programa("El elemento de la linea " + str(x+1) + " del archivo 'elementos' no coincide con la linea " + str(x+1) + " del archivo 'restricciones'")
 
 
guardar_log("Elementos y restricciones cargados correctamente en memoria")
...
# Creacion del archivo por_renovar.txt que contendra los modelos que satisfagan las condiciones de minimos
crear_archivo(nombre_archivo_por_renovar)
guardar_log("Creado archivo por_renovar.txt para empezar a aniadir los modelos que cumplan")

# Se escribe el encabezado para el archivo de por_renovar.txt
escribir_linea_archivo(nombre_por_renovar,"-------------------- MODELOS PARA EXHIBICION -------------------- \n \n")
...
+			# break al final y se asigna False a la variable, potente para que este modelo no se aniada al archivo por_renovar.txt
+			potente = False
...
+	
+	# Fuera del ciclo 5, se compara el booleano que tiene potente, si es True, entonces ese modelo se aniade a por_renovar.txt
+	if  potente:
+		escribir_linea_archivo(nombre_archivo_por_renovar, "El modelo %s, cumple para estar en exhibicion \n" % (tabla_total[0][p]))
